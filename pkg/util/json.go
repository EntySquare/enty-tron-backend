...
// InvestorsErrorResponse is a function that returns error responses in the standard Matrix Error format (errcode / error)
func InvestorsErrorResponse(httpStatusCode int, errCode, message string) JSONResponse {
	return JSONResponse{
		Code: httpStatusCode,
		JSON: struct {
...
func MakeJSONAPI(handler JSONRequestHandler) http.HandlerFunc {
	return WithCORSOptions(Protect(func(w http.ResponseWriter, req *http.Request) {
...
func SetCORSHeaders(w http.ResponseWriter) {
	...
	w.Header().Set("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept, Authorization, access-token")
}
...
func RandomHashSerialNumber() string {
	s := time.Now().Format("20060102150405.999999999") + util.RandomString(15)
	h := sha256.New()
	h.Write([]byte(s))
	bs := h.Sum(nil)
...
func HashMd5AndSha256(password string) string {
	s := md5.New()
