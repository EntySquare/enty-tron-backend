...
// EntyPayError represents the "standard error response" in Matrix.
// http://matrix.org/docs/spec/client_server/r0.2.0.html#api-standards
type EntyPayError struct {
	ErrCode string `json:"errcode"`
	Err     string `json:"error"`
}

func (e EntyPayError) Error() string {
...
func Unknown(msg string) *EntyPayError {
	return &EntyPayError{"M_UNKNOWN", msg}
}

// Forbidden is an error when the client tries to access a resource
// they are not allowed to access.
func Forbidden(msg string) *EntyPayError {
	return &EntyPayError{"M_FORBIDDEN", msg}
}

// BadJSON is an error when the client supplies malformed JSON.
func BadJSON(msg string) *EntyPayError {
	return &EntyPayError{"M_BAD_JSON", msg}
}

// BadRPC is an error when the client supplies malformed JSON.
func BadRPC(msg string) *EntyPayError {
	return &EntyPayError{"M_BAD_RPC", msg}
}

// NotJSON is an error when the client supplies something that is not JSON
// to a JSON endpoint.
func NotJSON(msg string) *EntyPayError {
	return &EntyPayError{"M_NOT_JSON", msg}
}

// NotFound is an error when the client tries to access an unknown resource.
func NotFound(msg string) *EntyPayError {
	return &EntyPayError{"M_NOT_FOUND", msg}
}

// MissingArgument is an error when the client tries to access a resource
// without providing an argument that is required.
func MissingArgument(msg string) *EntyPayError {
	return &EntyPayError{"M_MISSING_ARGUMENT", msg}
}

// InvalidArgumentValue is an error when the client tries to provide an
// invalid value for a valid argument
func InvalidArgumentValue(msg string) *EntyPayError {
	return &EntyPayError{"M_INVALID_ARGUMENT_VALUE", msg}
}
...
func MissingToken(msg string) *EntyPayError {
	return &EntyPayError{"M_MISSING_TOKEN", msg}
}
...
func UnknownToken(msg string) *EntyPayError {
	return &EntyPayError{"M_UNKNOWN_TOKEN", msg}
}
...
func WeakPassword(msg string) *EntyPayError {
	return &EntyPayError{"M_WEAK_PASSWORD", msg}
}

// InvalidUsername is an error returned when the client tries to register an
// invalid username
func InvalidUsername(msg string) *EntyPayError {
	return &EntyPayError{"M_INVALID_USERNAME", msg}
}

// UserInUse is an error returned when the client tries to register an
// username that already exists
func UserInUse(msg string) *EntyPayError {
	return &EntyPayError{"M_USER_IN_USE", msg}
}
...
// namespace.
func ASExclusive(msg string) *EntyPayError {
	return &EntyPayError{"M_EXCLUSIVE", msg}
}

// GuestAccessForbidden is an error which is returned when the client is
// forbidden from accessing a resource as a guest.
func GuestAccessForbidden(msg string) *EntyPayError {
	return &EntyPayError{"M_GUEST_ACCESS_FORBIDDEN", msg}
}
...
func UnsupportedRoomVersion(msg string) *EntyPayError {
	return &EntyPayError{"M_UNSUPPORTED_ROOM_VERSION", msg}
}
...
func LimitExceeded(msg string, retryAfterMS int64) *LimitExceededError {
	return &LimitExceededError{
		EntyPayError: EntyPayError{"M_LIMIT_EXCEEDED", msg},
...
func NotTrusted(serverName string) *EntyPayError {
	return &EntyPayError{
